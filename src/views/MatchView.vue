...
<template>
  <div v-if="match && p1 && p2" class="h-screen flex flex-col overflow-hidden bg-slate-950 p-3 text-white sm:p-4">
    ...
    <div class="grid min-h-0 flex-1 grid-cols-1 grid-rows-2 gap-2 pb-2 landscape:grid-cols-2 landscape:grid-rows-1 sm:grid-cols-2 sm:grid-rows-1">
      <section
        class="relative flex h-full min-h-0 flex-col overflow-hidden rounded-3xl transition-all duration-300 sm:rounded-[2.5rem]"
        :class="
        ...
        "
      >
        <div class="flex items-center justify-between border-b border-white/5 bg-red-500/10 px-3 py-2.5 sm:px-4 sm:py-3.5">
          <div class="min-w-0 flex-1">
            <h2 class="truncate text-xs font-black uppercase italic sm:text-sm">
              {{ p1.name }}
            </h2>
            <p class="truncate text-[9px] font-bold italic text-red-400/70 sm:text-[10px]">
        ...
        <div class=" min-h-0 flex-1 items-center justify-center flex landscape:flex">
          <span
            class="text-7xl text-red-400 font-black italic tracking-tighter tabular-nums landscape:text-4xl sm:text-7xl"
          >
            {{ match.p1_score }}
          </span>
        </div>

        <div class="mt-1.5 grid grid-cols-2 gap-1.5 bg-black/20 p-1.5 landscape:mt-auto sm:mt-2 sm:gap-2 sm:p-2">
          <button
            v-for="a in actions"
            :key="a.key + '-p1'"
            type="button"
            class="relative flex h-12 flex-col items-center justify-center overflow-hidden rounded-lg transition-all active:scale-95 sm:h-16 sm:rounded-2xl"
            :class="[
          ...
          >
            <span class="text-[9px] font-black uppercase italic text-white/80 sm:text-[10px]">
              {{ t(a.labelKey) }}
            </span>
      ...
      <section
        ...
        "
      >
        <div class="flex items-center justify-between border-b border-white/5 bg-blue-500/10 px-3 py-2.5 sm:px-4 sm:py-3.5">
          <div class="min-w-0 flex-1">
            <h2
              class="truncate text-xs font-black uppercase italic sm:text-sm"
              :class="
            ...
            <p
              class="truncate text-[9px] font-bold italic sm:text-[10px]"
              :class="
                match.status === 'finished' && match.p2_score < match.p1_score
                  ? 'text-red-400/70'
                  : 'text-blue-400/70'
        ...
        <div class="min-h-0 flex-1 items-center justify-center flex landscape:flex">
          <span
            class="text-7xl font-black italic tracking-tighter tabular-nums landscape:text-4xl sm:text-7xl"
            :class="
              match.status === 'finished' && match.p2_score < match.p1_score
                ? 'text-red-400'
                : 'text-blue-400'
    ...
    <transition name="winner">
      ...
      >
        ...
        <div class="relative w-full max-w-sm rounded-4xl border border-white/10 bg-slate-900 p-4 text-center shadow-2xl sm:p-8">
          <h3 class="mb-1 text-3xl font-black uppercase italic leading-none tracking-tighter text-white sm:text-5xl">
            {{ t('match.finished') }}
          </h3>
          <p class="mb-4 text-xs font-bold text-slate-400 sm:mb-8 sm:text-base">
            {{ t('match.winnerIs', { name: match.p1_score > match.p2_score ? p1.name : p2.name }) }}
          </p>
          <button
            type="button"
            class="w-full rounded-xl bg-white py-3 text-xs font-black uppercase italic tracking-widest text-black active:scale-95 sm:rounded-2xl sm:py-4 sm:text-base"
            @click="back"
          >
            {{ t('match.back') }}
          </button>
