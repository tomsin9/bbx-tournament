...
import {
  comboLabel,
  normalizeBeys,
  participantComboParts as playerComboParts,
  profilePreviewComboParts,
} from '@/utils/combo'

const props = defineProps<{
  useDraft?: boolean
  draftPlayers?: TournamentParticipant[]
...
const emit = defineEmits<{
  'update:draftPlayers': [value: TournamentParticipant[]]
}>()
...
const useDraft = computed(() => Boolean(props.useDraft))
const rosterPlayers = computed(() => (useDraft.value ? (props.draftPlayers ?? []) : store.players))
...
const slotCount = computed(() => {
  const configured = Number(props.maxBeysPerPlayer ?? store.maxBeysPerPlayer)
...
function submitPlayer() {
  ...
  if (!useDraft.value && libraryEditingId.value) {
    ...
    else if (beys.length > 0) nextCombos.unshift(nextCombo)
  ...
  if (useDraft.value) {
    ...
    const existing = rosterPlayers.value.find((p) => p.id === id)
    const player_id = existing?.player_id ?? `pl-${crypto.randomUUID().slice(0, 8)}`
    const next = [...rosterPlayers.value]
...
function deleteEditingProfileCombo(combo: Combo): void {
  ...
  if (!profile || useDraft.value) return
...
function removePlayer(id: string) {
  ...
  if (useDraft.value) {
    emit(
      'update:draftPlayers',
      rosterPlayers.value.filter((p) => p.id !== id),
    )
...
function quickAddFromLibrary(profile: PlayerProfile) {
  ...
  const defaultComboBeys = (profile.combos?.[0]?.beys ?? [])
    .map((bey) => bey.trim())
    .filter(Boolean)
  const defaultBey =
    (defaultComboBeys[0] ?? profile.default_bey_name ?? profile.bey_combos?.[0])?.trim() ||
    undefined
  const chosenBeys = defaultComboBeys.length > 0 ? defaultComboBeys : defaultBey ? [defaultBey] : []
  if (useDraft.value) {
    const today = new Date().toISOString().slice(0, 10)
    emit('update:draftPlayers', [
      ...rosterPlayers.value,
      {
        ...
        name: profile.name,
        bey_name: defaultBey,
        beys: chosenBeys.length ? chosenBeys : undefined,
        created_at: profile.created_at ?? today,
      },
    ])
    return
  }
  store.addPlayersFromLibrary([{ profile, bey_name: defaultBey, beys: chosenBeys }])
}

function startEditLibraryPlayer(id: string) {
  if (useDraft.value) return
              ...
              <div v-if="(editingId || libraryEditingId) && editingProfileCombos.length" class="space-y-1">
                ...
                <div class="flex flex-wrap gap-1.5">
                  <div
                    ...
                    :key="'shared-edit-combo-' + combo.id"
      ...
      <section class="order-2 lg:order-2 lg:col-span-7 space-y-4">
        <div class="flex min-h-6 items-center justify-between px-2">
          <h3 class="leading-none text-[10px] font-black uppercase tracking-[0.3em] text-bx-primary">
            {{ rosterTitle || t('setup.currentRoster') }} ({{ rosterPlayers.length }})
          </h3>
        ...
        <div class="h-[500px]">
          ...
          >
            <p class="font-bold italic uppercase tracking-widest text-slate-600">{{ emptyHintText || t('players.noPlayersReady') }}</p>
          </div>
          ...
          <transition-group v-else name="list" tag="ul" class="grid h-full gap-2 overflow-y-scroll custom-scrollbar pr-1">
            ...
            >
              <div class="flex items-center gap-4">
                <div>
                  <p class="font-black italic uppercase text-white leading-none inline-block">{{ p.name }}</p>
                  <p class="font-mono text-[10px] font-semibold tracking-wide text-slate-500 inline-block ml-1">
                    ({{ t('players.shortId', { id: shortPlayerIdSuffix(p.player_id) }) }})
                  </p>
                  <div class="mt-1 flex flex-wrap gap-1">
                    <span
                      v-for="(part, idx) in playerComboParts(p)"
                      :key="'shared-roster-combo-' + p.id + '-' + idx"
                    ...
                    <div class="space-y-1">
                      ...
                      <div class="flex flex-wrap gap-1">
                        <span
                          ...
                          :key="'shared-lib-combo-preview-' + p.id + '-' + idx"
